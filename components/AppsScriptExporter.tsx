...
const AppsScriptExporter: React.FC = () => {
  ...
  const scriptCode = `/**
 * EDUSTREAM PRO - ENTERPRISE CLOUD SYNC ENGINE v8.0 (FULL PORTAL)
 * Optimized for Marks, Notifications, and Multi-Role Portals
 */
...
const DB_CONFIG = {
  ...
  FEE_LEDGER: {
    ...
    // Added Base_Amount, Fine_Amount, and Fine_Reason to headers
...
function onOpen() {
  SpreadsheetApp.getUi().createMenu('🏫 EduStream ERP v8').addItem('🚀 Setup All Sheets', 'initializeEnterpriseSheets').addToUi();
}
...
function doPost(e) {
  try {
    ...
    // Updated fee mapping to include baseAmount, fineAmount, and fineReason
...
function initializeEnterpriseSheets() {
  ...
  SpreadsheetApp.getUi().alert('✅ EduStream v8 Database Ready!');
}
`;
    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-16 animate-in fade-in duration-700">
      <div className="bg-white p-8 rounded-[3.5rem] border border-slate-100 shadow-xl flex flex-col md:flex-row items-center justify-between gap-6 ring-1 ring-slate-100">
        <div className="flex items-center gap-6">
          ...
          <div><h2 className="text-3xl font-black text-slate-900 tracking-tight">Portal Sync Engine</h2><p className="text-indigo-600 font-bold tracking-tight text-sm uppercase">Version 8.0 Enterprise Build</p></div>
        </div>
        <button onClick={handleCopy} className={`px-10 py-5 rounded-[2rem] font-black text-xs uppercase tracking-widest flex items-center gap-2 transition-all shadow-2xl active:scale-95 ${copied ? 'bg-emerald-500 text-white' : 'bg-slate-900 text-white hover:bg-indigo-600'}`}>
          {copied ? <CheckCircle size={20} /> : <FileCode size={20} />}{copied ? 'Code Copied!' : 'Copy v8.0 Source Code'}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="bg-indigo-900 rounded-[3rem] p-10 text-white relative overflow-hidden h-fit shadow-2xl">
             ...
             <h3 className="text-2xl font-black mb-8 text-indigo-200">v8 Deployment Guide</h3>
             <div className="space-y-6">
                <StepItem num="1" text="पुराना Apps Script कोड पूरी तरह डिलीट करें।" />
                <StepItem num="2" text="नया v8.0 कोड यहाँ से कॉपी करके पेस्ट करें।" />
                <StepItem num="3" text="नया Deployment करें (Who has access: Anyone)।" />
                <StepItem num="4" text="नया URL Settings में डालकर 'Push' करें।" />
             </div>
        </div>
        <div className="lg:col-span-2 bg-slate-950 rounded-[3rem] overflow-hidden flex flex-col h-[600px] border border-slate-800 shadow-2xl">
             <div className="p-6 bg-slate-900 border-b border-slate-800 flex items-center justify-between">
                ...
                <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">enterprise_sync_v8.gs</span>
             </div>
