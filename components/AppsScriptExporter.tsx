...
const DB_CONFIG = {
  STUDENT_MASTER: {
    ...
    headers: ['ID', 'Admission_No', 'Roll_No', 'First_Name', 'Last_Name', 'Gender', 'DOB', 'Email', 'Class', 'Section', 'Father_Name', 'Father_Mobile', 'Address', 'City', 'Status', 'Admission_Date', 'Aadhar_No', 'Previous_School', 'Previous_Grade', 'TC_Number']
  },
  FEE_LEDGER: {
...
function doPost(e) {
  try {
    ...
    if (payload.students) syncData('STUDENT_MASTER', payload.students, s => [s.id, s.admissionNo, s.rollNo || '', s.firstName, s.lastName, s.gender, s.dob, s.email || '', s.admissionClass, s.section || '', s.fatherName, s.fatherMobile, s.address || '', s.city || '', s.status, s.admissionDate, s.aadharNo || '', s.previousSchool || '', s.previousGrade || '', s.tcNumber || '']);
