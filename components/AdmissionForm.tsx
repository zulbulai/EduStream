...
import { User, Users, BookOpen, Camera, FileText, CheckCircle2, MapPin, PhoneCall, School, Check, ShieldCheck } from 'lucide-react';
...
const AdmissionForm: React.FC = () => {
  ...
  const handleSubmit = async () => {
    if (!formData.firstName || !formData.lastName || !formData.admissionClass || !formData.fatherMobile) {
      alert("Please fill mandatory fields (Name, Class, Mobile)");
      return;
    }
  ...
  if (success) {
    return (
      <div className="max-w-xl mx-auto py-20 text-center space-y-6 animate-in zoom-in duration-500">
        ...
        <p className="text-slate-500 font-medium">Student <span className="text-indigo-600 font-bold">{formData.firstName}</span> registered for Academic Session 2025-26.</p>
        <button onClick={() => window.location.reload()} className="px-10 py-4 bg-indigo-600 text-white rounded-[2rem] font-black text-sm uppercase tracking-widest shadow-2xl hover:bg-indigo-700 transition-all active:scale-95">Enroll Another Student</button>
      </div>
    );
  }
    ...
    <div className="max-w-5xl mx-auto space-y-10 pb-20 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">Official Admission Management Portal</p>
        </div>
      ...
      <div className="bg-white rounded-[3.5rem] shadow-2xl border border-slate-100 overflow-hidden">
        <div className="p-12">
            ...
            <div className="space-y-10 animate-in slide-in-from-right-8 duration-500">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="flex flex-col items-center space-y-4">
                    ...
                    <p className="text-[10px] font-black text-slate-400 uppercase">Upload Passport Photo</p>
                </div>
                <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">First Name *</label>
                    <input name="firstName" onChange={handleInputChange} className="w-full px-6 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none focus:border-indigo-600 font-bold" placeholder="First Name" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Last Name *</label>
                    <input name="lastName" onChange={handleInputChange} className="w-full px-6 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none focus:border-indigo-600 font-bold" placeholder="Last Name" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Gender</label>
                    <select name="gender" onChange={handleInputChange} className="w-full px-6 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none focus:border-indigo-600 font-bold">
                      <option>Select</option><option>Male</option><option>Female</option><option>Other</option>
                    </select>
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Date of Birth</label>
                    <input name="dob" type="date" onChange={handleInputChange} className="w-full px-6 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none focus:border-indigo-600 font-bold" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-10 animate-in slide-in-from-right-8 duration-500">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="col-span-2 space-y-4">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 block">Assigned Subjects</label>
                    <div className="flex flex-wrap gap-2">
                        {SUBJECT_LIST.map(sub => (
                            <button 
                                ...
                                className={`px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-tight transition-all border-2 ${
          ...
          {step === 4 && (
            <div className="space-y-10 animate-in slide-in-from-right-8 duration-500">
              <div className="flex items-center gap-4 pb-6 border-b border-slate-50">
                <div className="w-12 h-12 bg-indigo-50 text-indigo-600 rounded-2xl flex items-center justify-center"><MapPin size={24} /></div>
                <h3 className="text-2xl font-black text-slate-900">Location & Emergency</h3>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="col-span-2 space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Full Permanent Address</label>
                    <textarea name="address" onChange={handleInputChange} rows={2} className="w-full px-6 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none focus:border-indigo-600 font-bold resize-none" />
                </div>
                <div className="space-y-2">
          ...
          {step === 5 && (
            <div className="space-y-10 animate-in slide-in-from-right-8 duration-500 text-center">
                <div className="w-20 h-20 bg-indigo-50 text-indigo-600 rounded-full flex items-center justify-center mx-auto mb-6"><ShieldCheck size={40} /></div>
                ...
                <div className="bg-slate-50 p-8 rounded-[3rem] text-left grid grid-cols-2 gap-6 max-w-2xl mx-auto border border-slate-100">
                    <div><p className="text-[9px] font-black text-slate-400 uppercase">Student Name</p><p className="font-black">{formData.firstName} {formData.lastName}</p></div>
                    <div><p className="text-[9px] font-black text-slate-400 uppercase">Admission Class</p><p className="font-black">{formData.admissionClass} - {formData.section}</p></div>
                    <div><p className="text-[9px] font-black text-slate-400 uppercase">Mobile</p><p className="font-black">{formData.fatherMobile}</p></div>
                    <div><p className="text-[9px] font-black text-slate-400 uppercase">Subjects</p><p className="font-bold text-xs">{(formData.subjects || []).join(', ')}</p></div>
                </div>
                <button onClick={handleSubmit} disabled={isSubmitting} className="px-16 py-5 bg-slate-900 text-white rounded-[2.5rem] font-black text-sm uppercase tracking-widest shadow-2xl hover:bg-indigo-600 transition-all">
        ...
        <div className="px-12 py-8 bg-slate-50/50 border-t border-slate-100 flex items-center justify-between">
          <button onClick={() => setStep(prev => prev - 1)} disabled={step === 1} className="text-slate-400 font-black uppercase text-[10px] tracking-widest hover:text-slate-900 disabled:opacity-0 transition-all">Back</button>
          {step < totalSteps && (
            <button onClick={() => setStep(prev => prev + 1)} className="px-12 py-4 bg-indigo-600 text-white rounded-[2rem] font-black text-xs uppercase tracking-widest shadow-xl hover:bg-indigo-700 transition-all active:scale-95">Continue</button>
          )}
        </div>
      </div>
    </div>
